...
describe("useDropzone() hook", () => {
  ...
  describe("onClick", () => {
    ...
    it("should stop event propagations to the parent on clicks if {noClickEventsBubbling} is true", async () => {
      const onParentClick = jest.fn();
      const onDropClick = jest.fn();

      const InnerDropzone = () => (
        <Dropzone noClickEventsBubbling>
          {({ getRootProps, getInputProps }) => (
            <div id="inner-dropzone" {...getRootProps()}>
              <input {...getInputProps()} />
              <button onClick={onDropClick} />
            </div>
          )}
        </Dropzone>
      );
    ...
    it("does not trigger the click event on the input if event propagation was stopped", () => {
      ...
      const { container } = render(
        <Dropzone>
          {({ getRootProps, getInputProps }) => (
            <div
              {...getRootProps({ onKeyDown: event => event.stopPropagation() })}
            >
              <input {...getInputProps()} />
            </div>
          )}
